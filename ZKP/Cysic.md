...
Cysic (Crypto-Systemic Identity & Computation) 是一个专注于零知识证明（ZKP）硬件加速的领先项目。它致力于通过专用的硬件设计（FPGA 和 ASIC）和软硬件协同优化，大幅提升 ZKP 生成的速度和效率，旨在成为 ZK 生态系统的底层算力基础设施。

## 要解决的问题

随着 ZK-Rollup 和 zkEVM 的兴起，零知识证明的生成（Proving）过程成为了整个系统的性能瓶颈。

1.  **计算密集型**：生成 ZKP 涉及大量的密码学运算，如多标量乘法（MSM）和快速傅里叶变换（NTT），这对通用 CPU 来说极其繁重。
2.  **高延迟与高成本**：在普通硬件上生成复杂的 zkEVM 证明可能需要数十分钟甚至数小时，且不仅消耗大量电力，还导致 Layer 2 的最终确认时间变长。
...
2.  **专用 ASIC 芯片**：相比于 FPGA，ASIC 芯片在功耗和性能上具有更大的优势。Cysic 正在研发专用的 ZK 挖矿芯片，旨在像比特币矿机一样，通过特定的硬件高效产出证明。
...
*   **广泛兼容性**：支持主流的证明系统（如 Plonk, Groth16, Halo2）和椭圆曲线（如 BN254, BLS12-381），可服务于 Scroll, zkSync, Aztec 等多个 ZK-Rollup 项目。
