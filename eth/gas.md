...
Gas（燃料）是以太坊网络中用于衡量执行特定操作（如交易转账、智能合约计算、存储数据）所需计算工作量的单位。它是以太坊经济模型和安全机制的核心。

## 要解决的问题

作为去中心化的世界计算机，以太坊面临两个关键的资源管理问题：

1.  **停机问题（Halting Problem）**：由于 EVM 是图灵完备的，恶意或错误的代码可能包含无限循环。如果没有终止机制，网络节点将永远陷入死循环，导致网络瘫痪。
...
### Gas 计量
EVM 中的每个操作码（Opcode）都有固定的 Gas 成本（例如，`ADD` 消耗 3 Gas，`SSTORE` 写入存储可能消耗 20000 Gas）。复杂的交易是所有子操作 Gas 消耗的总和。

### EIP-1559 费用模型
自 2021 年伦敦升级（EIP-1559）以来，以太坊采用了新的费用市场机制：
...
*   **Gas Limit (Block)**：单个区块允许包含的最大 Gas 总量，决定了区块的大小上限。
*   **MEV (最大可提取价值)**：验证者通过重新排序交易来获取额外收益，与 Gas 竞价密切相关。
