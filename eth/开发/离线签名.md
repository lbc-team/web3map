...
### 解决的问题
传统的[以太坊](https://learnblockchain.cn/tags/以太坊?map=EVM)交互要求用户为每一个操作（如授权代币、投票）发起一笔交易并支付 Gas 费。这导致了以下问题：
*   **用户体验差:** 用户需要等待交易确认。
*   **成本高:** 简单的意图表达（如投票）也需要支付 Gas。
*   **无法代付 [Gas](https://learnblockchain.cn/tags/Gas?map=EVM):** 发起交易的[账户](https://learnblockchain.cn/tags/账户?map=EVM)必须持有 ETH。

离线签名技术通过允许用户在链下签署数据（意图），然后由他人（Relayer）提交上链，解决了上述问题。这实现了“元交易 (Meta-Transactions)”和“无 [Gas](https://learnblockchain.cn/tags/Gas?map=EVM) 交易 (Gasless Transactions)”。

### 实现机制与原理
[以太坊](https://learnblockchain.cn/tags/以太坊?map=EVM)主要的离线签名标准包括：
...
### 推荐阅读
*   [EIP-712: Typed structured data hashing and signing](https://learnblockchain.cn/docs/eips/EIPS/eip-712)
*   [EIP-2612: Permit Extension for ERC20](https://learnblockchain.cn/docs/eips/EIPS/eip-2612)

### 相关概念
*   **ECDSA:** [椭圆曲线](https://learnblockchain.cn/tags/%E6%A4%AD%E5%9C%86%E6%9B%B2%E7%BA%BF)数字签名算法，[以太坊](https://learnblockchain.cn/tags/以太坊?map=EVM)使用的签名算法。
*   **ecrecover:** [Solidity](https://learnblockchain.cn/tags/Solidity?map=EVM) 内置函数，用于从签名中恢复出签名者的地址。
