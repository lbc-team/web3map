...
ethers.js 是一个完整、紧凑的 JavaScript 库,用于与以太坊区块链进行交互。由 Richard Moore（ricmoo）于2016年创建,ethers.js 以其简洁的 API、完善的文档、强大的 TypeScript 支持和安全的设计理念,成为以太坊开发者的首选库之一。

ethers.js 提供了连接以太坊节点、管理钱包、与智能合约交互、签名交易等全方位的功能。相比更早的 Web3.js,ethers.js 采用了更现代的设计,体积更小（压缩后约88KB）、API 更简洁、类型安全更好。它被广泛应用于 DApp 前端、Node.js 脚本、钱包应用等场景,支撑着以太坊生态中数以万计的项目。
...
Provider 提供只读的链上数据访问,如查询余额、获取区块、估算 Gas 等。
...
- **合约实例化**：通过合约地址、ABI 和 Provider/Signer 创建合约对象
...
**DApp 前端开发**：几乎所有以太坊 DApp 的前端都使用 ethers.js 或 Web3.js 与链交互。ethers.js 轻量、易用,是首选库。
...
**测试和部署**：Hardhat、Foundry 等开发框架集成 ethers.js 用于合约测试和部署脚本。
...
- **Web3.js** - 另一个主流以太坊 JavaScript 库
- **viem** - 新兴的 TypeScript 优先以太坊库
- **Hardhat** - 集成 ethers.js 的智能合约开发框架
- **MetaMask** - 可与 ethers.js 配合使用的浏览器钱包
- **WalletConnect** - 支持 ethers.js 的钱包连接协议
- **RainbowKit** - 基于 ethers.js/wagmi 的钱包连接组件
- **ERC712** - ethers.js 支持的结构化数据签名标准
