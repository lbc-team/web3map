...
- **Rebase 机制**：stETH 是一种 Rebase 代币。随着以太坊验证者获得质押奖励，stETH 的总供应量会每天自动调整（增加），用户的钱包余额也会相应增加。这意味着 1 stETH 的价格理论上应始终锚定 1 ETH。
- **wstETH (Wrapped stETH)**：为了兼容某些不支持 Rebase 机制的 DeFi 协议（如 Uniswap, MakerDAO），Lido 提供了封装版本的 wstETH。wstETH 的余额不变，但其对 stETH 的兑换率会随时间增长（价值累积型）。
...
### 3. 提款与缓冲
在以太坊上海升级后，Lido 支持 ETH 赎回。协议维护一个提款缓冲区，通过用户新存入的 ETH 和部分奖励来满足提款需求，若缓冲区不足则启动验证者退出流程。

## 主要特点

- **极佳的流动性**：stETH 是 DeFi 中流动性最好的质押资产，在 Curve 等 DEX 上拥有深厚的 stETH/ETH 池，几乎可以无损退出。
- **DeFi 乐高基石**：stETH 被广泛接受为 Aave 的抵押品、MakerDAO 的铸币资产等，极大地提高了 ETH 的资本效率。
- **简单易用**：用户一键质押，无需维护硬件。

## 争议与挑战

- **中心化风险**：Lido 占据了以太坊质押市场极高的份额（曾超过 30%），引发了社区对于网络中心化和审查风险的担忧。Lido 正在通过引入 DVT（分布式验证技术）和模块化节点运营商集合（Staking Router）来试图缓解这一问题。
