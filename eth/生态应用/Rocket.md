...
与 [Lido](https://learnblockchain.cn/tags/Lido?map=EVM) 等其他流动性质押方案相比，Rocket Pool 最显著的特点是其无需许可（Permissionless）的节点运营机制。任何只要满足最低 ETH 和 RPL（协议代币）抵押要求的用户，都可以成为节点运营商（Node Operator），为协议运行验证节点。

Rocket Pool 于 2021 年底上线主网，一直致力于维护[以太坊](https://learnblockchain.cn/tags/以太坊?map=EVM)的去中心化和抗审查属性。
...
- **价值累积**：与 [Lido](https://learnblockchain.cn/tags/Lido?map=EVM) 的 stETH（rebase 机制，数量增加）不同，rETH 采用增值模型。rETH 的数量不变，但其对 ETH 的兑换汇率会随着质押奖励的积累而不断上升。
...
- **[以太坊](https://learnblockchain.cn/tags/以太坊?map=EVM)要求**：传统 Solo Staking 需要 32 ETH。在 Rocket Pool 中，节点运营商只需提供 8 ETH 或 16 ETH（取决于配置，LEB8 或 LEB16）。
