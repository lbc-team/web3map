...
2.  **Bundler (打包者):** 这是一个运行特定节点的角色（可以是矿工/验证者）。它监听 UserOp 内存池，将多个 UserOperation 打包成一笔普通的以太坊交易。
3.  **EntryPoint (入口合约):** 一个全局单例合约。Bundler 将打包好的交易发送给 EntryPoint。EntryPoint 负责验证 UserOperation 并执行智能合约钱包中的操作。
4.  **Smart Account (智能账户):** 用户的钱包合约。它必须实现验证逻辑（验证 UserOp 的签名）和执行逻辑。
5.  **Paymaster (代付合约):** 可选组件。允许第三方（DApp 项目方或服务商）为用户支付 Gas 费，或者允许用户使用 ERC-20 代币支付 Gas。
6.  **Aggregator (聚合器):** 可选组件。用于支持 BLS 等聚合签名，进一步节省 Gas。
