...
**强制租金豁免**
自 2021 年起，[Solana](https://learnblockchain.cn/tags/Solana?map=Solana) 要求所有新账户必须达到租金豁免标准：
- 创建账户时必须存入足够 SOL
- 不豁免的账户无法创建
- 已存在的非豁免账户逐步被回收
...
| 链 | 存储成本模型 |
|------|------------|
| [Solana](https://learnblockchain.cn/tags/Solana?map=Solana) | 一次性支付（租金豁免） |
| Ethereum | [Gas](https://learnblockchain.cn/tags/Gas?map=EVM) 费 + 永久存储 |
| Near | 质押代币锁定存储 |
| Arweave | 一次性支付永久存储 |
...
**2. 使用 [PDA](https://learnblockchain.cn/tags/PDA?map=Solana)**
合并多个用户数据到一个账户。
...
- **Epoch**：[Solana](https://learnblockchain.cn/tags/Solana?map=Solana) 的时间周期，约 2-3 天
